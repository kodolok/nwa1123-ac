 ...
 * All rights reserved.
 *
 * $Id: //depot/sw/releases/Aquila_9.2.0_U10/apps/busybox-1.01/miscutils/ethreg.c#1 $
 */
...
static void athr_gmac_led_ctrl(int val)
{
	etd.val = val;
	etd.cmd = ATHR_GMAC_LED_CTRL;
	if (ioctl (s, ATHR_GMAC_CTRL_IOC, &ifr) < 0)
		printf("%s ioctl error\n",__func__);
}
...
static void usage(void)
{
	...
	fprintf(stderr, "usage: %s [-f]  -p portnum =10/100/0 [-d duplex]\n", progname);
        ...
        fprintf(stderr, "usage: %s [--ledctl] [-i ifname] -v [0|1]\n", progname);
...
ethreg_main(int argc, char *argv[])
{
	...
	struct option long_options[] =
        {
            ...
            { "ledctl" , no_argument, 0, ATHR_GMAC_LED_CTRL},
	...
	switch (c) {
        	...
        	case ATHR_PHY_TXFCTL:
                ...
                case ATHR_PHY_FLOW_CTRL:
                        ...
                        break;
                case ATHR_GMAC_LED_CTRL:
                        cmd = ATHR_GMAC_LED_CTRL;
                        break;
                case 'm':
                        mac = optarg;
                        break;
	...
	strncpy(etd.ad_name, ifname, sizeof (etd.ad_name));
        ...
        else if (cmd == ATHR_GMAC_LED_CTRL){
        	athr_gmac_led_ctrl(value);
                return 0;
        }
