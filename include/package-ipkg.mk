...
ifeq ($(DUMP),)
  define BuildTarget/ipkg
    ...
    ifeq ($(2),check-deps)
	IPRE_$(1):=$(PREBUILT_DIR)/$(1)_$(VERSION)_$(PKGARCH).ipk
	Build/InstallDev:=
    endif
...
ifeq ($(2),check-deps)
	...
	-find $$(IDIR_$(1)) -name 'CVS' -o -name '.gitignore' -o -name '.svn' -o -name '.#*' -o -name '*~'| $(XARGS) rm -rf
	@( \
	...
	( \
		...
		$(if $(PRIORITY),echo "Priority: $(PRIORITY)"; ) \
  ...
  define BuildTarget/ipkg-prebuilt-ipk
  ...
  define BuildTarget/ipkg-prebuilt
    ...
    ifeq ($(wildcard $(PREBUILT_DIR)/*.ipk),)
	$(BuildTarget/ipkg)
    else
      ifeq ($(CONFIG_PACKAGE_$(1)),y)
        $(BuildTarget/ipkg)
      else
	$(BuildTarget/ipkg/$(1)/check-deps)
      endif
     endif
    else
      $(BuildTarget/ipkg-prebuilt-ipk)
    endif
  endef
