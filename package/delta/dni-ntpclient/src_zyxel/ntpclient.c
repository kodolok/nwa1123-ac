...
/* straight out of RFC-1305 Appendix A */
	...
	int freq;
	char sys_name[MAX_LOG_STR_LEN], sys_name_modify[MAX_LOG_STR_LEN], log_str[MAX_LOG_STR_LEN];
#ifdef ENABLE_DEBUG
...
#undef FAIL
	...
	if (ntpc->set_clock) { /* you'd better be root, or ntpclient will exit here! */
		set_time(&xmttime);
		ExecuteGetCfg ("SYS_NAME", sys_name);
		...
		while(sys_name[i] != '\0')
		{
			if (sys_name[i] == '%')
			{
				...
				{
					system("echo \"ntp overflow!\"");
					break;
				}
			}
/*			else if (sys_name[i] == 0x22)    //    " is 0x22
			{
                                if (j <= MAX_LOG_STR_LEN - 2)
                                {
                                        sys_name_modify[j++] = 0x5c;
                                        sys_name_modify[j++] = 0x5c;
                                }
                                else
                                {
                        ...
                        else if (sys_name[i] == 0x5c)    //    \ is 0x5c
                        {
...
*/			
		...
		sprintf(log_str, "%s update time", sys_name_modify);
		syslog(LOG_INFO,log_str);
		system("echo 0 > /tmp/ntp_update");
	}
