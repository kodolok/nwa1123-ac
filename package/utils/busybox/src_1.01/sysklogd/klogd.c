...
static void klogd_signal(int sig)
{
	...
	syslog(LOG_LOCAL1, "Kernel log daemon exiting.");
	exit(EXIT_SUCCESS);
}
...
//	syslog(LOG_LOCAL1, "klogd started: " BB_BANNER);
	...
	while (1) {
		...
		if (n < 0) {
			if (errno == EINTR)
				continue;
			syslog(LOG_LOCAL0, "klogd: Error return from sys_sycall: %d - %m.\n", errno);
			exit(EXIT_FAILURE);
		}
