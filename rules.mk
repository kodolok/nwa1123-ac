...
export PKG_DIR_BASE:=$(TOPDIR)/package
DNI_CONFIG_CHECK:=`cat $(TOPDIR)/.config | grep _dni-config=y`
ifneq ($(DNI_CONFIG_CHECK),)
	export DNI_CONFIG:=+dni-config
	include $(TOPDIR)/.config
endif
...
BIN_DIR:=$(BIN_DIR_BASE)/$(BOARD)
INCLUDE_DIR:=$(TOPDIR)/include
SCRIPT_DIR:=$(TOPDIR)/scripts
PREBUILT_DIR:=$(TOPDIR)/target/linux/$(BOARD)/prebuilt/ipkg
...
ifeq ($(CONFIG_EXTERNAL_TOOLCHAIN),)
  ...
  TOOLCHAIN_DIR:=$(call qstrip,$(CONFIG_TOOLCHAIN_ROOT))
  PACKAGE_DIR:=$(BIN_DIR)/packages
endif

ifeq ($(GCC_VERSION),)
	ifneq ($(wildcard $(TOOLCHAIN_DIR)/info.mk),)
...
LIBGCC_S_PATH=$(call qstrip,$(CONFIG_LIBGCC_ROOT_DIR))/lib
...
ifndef DUMP
  ifeq ($(CONFIG_EXTERNAL_TOOLCHAIN),)
    ...
    ifeq ($(CONFIG_NATIVE_TOOLCHAIN),)
      ...
      TOOLCHAIN_DIR_NAME:=$(call qstrip,$(CONFIG_TOOLCHAIN_FILENAME))
